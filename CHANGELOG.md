...
- Repository, VS Code, and JetBrains descriptions, empty states, and Marketplace copy repositioned consistently as **Visual Asset Governance** — static assets are now presented as first-class citizens throughout, not a caveat.
- **animoria-jetbrains**: `DeleteAssetAction` now moves assets to `.animoria/trash/` via the daemon instead of permanently deleting them, matching every other destructive path in the product.
- **animoria-jetbrains**: `DuplicateResolverDialog` now requires an explicit "Resolve Duplicates" confirmation (previously a generic, unlabeled "OK") stating how many files will be moved and which copy is kept.
- **animoria-jetbrains**: diagnostics now also surface through the IDE's Event Log (a dedicated "Animoria" notification group), not only the global `idea.log`.
- **animoria-jetbrains**: the gallery now shows an explicit empty-state message immediately on load, instead of a blank tree until the first scan event arrives.
- **animoria-jetbrains**: the editor hover hint is now scoped to real file editors only, requires a minimum stem length, and is rate-limited — reducing false-positive and repeat-popup noise.

### Added

- **animoria-jetbrains**: JUnit5 test coverage for `AnimoriaTreeModel`'s state transitions (asset mutations, governance sections, duplicate groups, thumbnail states, view-mode toggling, empty state).

- Standard `typecheck`, `check`, and `clean` scripts across the workspace, and a `lint`/`typecheck` script for `animoria-sandbox`.
- CI now runs formatting, lint, and typecheck gates in addition to build and test.
- Shared `tsconfig.base.json` with stricter compiler options (`exactOptionalPropertyTypes`, `noUncheckedIndexedAccess`, `noImplicitOverride`), extended by all packages.
- **animoria-core**: `StaticAssetScanner` — discovers static SVG, PNG, JPEG, WebP, and AVIF files as a separate workspace inventory. Usage tracking and governance analysis (duplicate/unused detection) do not yet cover static assets.
- **animoria-jetbrains**: Feature parity with the VS Code client — native asset gallery, governance analysis and report export, thumbnail generation, code-snippet generation, duplicate resolution, Bulk Cleanup review, and a lightweight editor hover — all backed by `@animoria/core` through a background daemon process.
- **animoria-jetbrains**: The daemon ships as a self-contained native executable (no separate Node.js install required on the end user's machine), built per-platform from `@animoria/core`'s CLI.

### Changed

- **animoria-core**: Thumbnail rendering no longer depends on a headless Chromium install. Static previews are rendered natively inside the Node.js/Electron runtime the IDE already provides, using a browserless SVG renderer for Lottie vector content with a format-badge fallback.
- `packages/animoria-jetbrains/src/main/resources/assets/**` (a generated Vite build output) is no longer committed to git; it's regenerated by `pnpm package:jetbrains`.

### Fixed

- A `runIde`/`exportGovernanceReport` code path in `animoria-vscode` that used the first workspace folder without checking the workspace list was non-empty.
- A `no-case-declarations` lint violation in the sandbox mock extension host, previously unlinted because `animoria-sandbox` had no `lint` script.

### Removed

- The `animoria.chromiumPath` setting and the `puppeteer-core` dependency, no longer needed after the move to native thumbnail rendering.
...
[1.0.0]: https://github.com/sxnnyside-project/animoria/compare/v0.2.0...v1.0.0
